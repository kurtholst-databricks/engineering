...
# MAGIC - `criterion`: chooses between `gini` and `entroy`. Defining the criterion parameter allows the algorithm to try both options during tuning and can assist in identifing which criterion works best. [TPE](https://optuna.readthedocs.io/en/stable/reference/samplers/generated/optuna.samplers.TPESampler.html#optuna.samplers.TPESampler) is the [default](https://optuna.readthedocs.io/en/stable/tutorial/10_key_features/003_efficient_optimization_algorithms.html#pruning), though there are [other sampling methods](https://optuna.readthedocs.io/en/stable/reference/samplers/index.html) like GPSampler and BruteForceSampler.
...
# MAGIC
# MAGIC 1. Select experiment which has the same name as this notebook's title (**2.1 - Hyperparameter Tuning with Optuna**).
# MAGIC
# MAGIC 1. Click on the graph icon at the top left under **Run**.
# MAGIC
...
import pandas as pd
pdf = spark.read.table("dbacademy.labuser9701456_1742889973.diabetes_binary").toPandas()
...
import pandas as pd
pdf = spark.read.table("dbacademy.labuser9701456_1742889973.ca_housing").toPandas()
