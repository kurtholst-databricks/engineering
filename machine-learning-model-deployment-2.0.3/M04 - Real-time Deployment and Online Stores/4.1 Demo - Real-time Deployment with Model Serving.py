...
# MAGIC
# MAGIC First, we will fit a model **without using a feature store**. Then, we will serve the model via Model Serving. Model serving **supports both the API and the UI**. To demonstrate both methods, we will first serve the model using the UI, and then serve the model using **Databricks' Python SDK**.
# MAGIC
...
# MAGIC   
# MAGIC     b. Click in the Entity field. A dialog appears. Go to **My models**, and then select the catalog, schema, and model from the drop-down menus. 
# MAGIC   
...
dataframe_records = [
    {"SeniorCitizen": 0, "tenure":12, "MonthlyCharges":65, "TotalCharges":1800},
    {"SeniorCitizen": 1, "tenure":24, "MonthlyCharges":40, "TotalCharges":500},
    {"SeniorCitizen": 1, "tenure":24, "MonthlyCharges":25, "TotalCharges":810},
    {"SeniorCitizen": 1, "tenure":24, "MonthlyCharges":40, "TotalCharges":500}
]
...
# MAGIC
# MAGIC From the **Create** dropdown, select **Online table**.
# MAGIC
...
# MAGIC %md
# MAGIC
# MAGIC **Step 1: Enable Change Data Feed on the `features` table:**
# MAGIC
# MAGIC ```
# MAGIC spark.sql(f'ALTER TABLE {DA.catalog_name}.{DA.schema_name}.features SET TBLPROPERTIES (delta.enableChangeDataFeed = true)')
# MAGIC ```
# MAGIC
# MAGIC **Step 2: Define table configuration:**
# MAGIC
# MAGIC ```
# MAGIC from databricks.sdk.service.catalog import *
# MAGIC
# MAGIC # Create an online table
# MAGIC spec = OnlineTableSpec(
# MAGIC   primary_key_columns=[primary_key],
# MAGIC   source_table_full_name=feature_table_name,
# MAGIC   run_triggered=OnlineTableSpecTriggeredSchedulingPolicy.from_dict({'triggered': 'true'})
# MAGIC )
# MAGIC ```
# MAGIC
# MAGIC **Step 3: Create the table:**
# MAGIC
# MAGIC ```
...
# MAGIC
# MAGIC online_table = OnlineTable(
# MAGIC   name=f"{DA.catalog_name}.{DA.schema_name}.online_features",  # Fully qualified table name
# MAGIC   spec=spec  # Online table specification
# MAGIC )
# MAGIC
# MAGIC w.online_tables.create_and_wait(table=online_table)
# MAGIC ```
